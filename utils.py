...
def fn_peformance(df_month, this_month):
    ...
    # -----------------------------------------------  보험회사별 랭킹  -----------------------------------------------------------
    # 보험회사별 매출액 상위 지점
    dfr_com_ptn = fn_visualization(df_month, ['파트너','소속','보험회사'], 'rank')
    lst_com_ptn = fn_ranking_normal(dfr_com_ptn, '보험회사', ['파트너','소속'])
    # 보험회사별 매출액 상위 FA
    # 보험회사별 매출액 상위 보험상품
    ...
    if cat[1].toggle("상품군별 매출액 상위 지점 (완)"):
        st.markdown("##### 상품군별 매출액 상위 부문")
        fn_toggle(lst_cat_ptn, 'multiple')
    if cat[2].toggle("상품군별 매출액 상위 FA (완)"):
