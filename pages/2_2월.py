...
if authentication_status:
    ...
    ##########################################################################################################################
    # -------------------------------------------  매출액 기준 기본 전처리  ----------------------------------------------------
    dfc_insu = fn_visualization(df_feb, ['보험종목','영수일자'], 'chart') # 보험종목별 매출액
    dfc_company = fn_visualization(df_feb, ['보험회사','영수일자'], 'chart') # 보험회사별 매출액
    dfc_product = fn_visualization(df_feb, ['상품군','영수일자'], 'chart') # 상품군별 매출액
    dfc_channel = fn_visualization(df_feb, ['소속','영수일자'], 'chart') # 소속부문별 매출액
    dfc_insu = fn_insurance(df_feb, dfc_insu) # 보험종목별(손생) 매출액 데이터에 합계 데이터 삽입: ['보험종목','영수/환급일','매출액']

    # ----------------------------------------------------  랭킹  -----------------------------------------------------------
    dfr_chn = fn_visualization(df_feb, ['소속'], 'rank') # 소속부문 매출액 순위
    dfr_fa = fn_visualization(df_feb, ['담당자코드','담당자','파트너'], 'rank') # FA 매출액 순위
    dfr_fa = dfr_fa.drop(columns='담당자코드')
    dfr_com = fn_visualization(df_feb, ['보험회사'], 'rank') # 보험회사 매출액 순이
    dfr_cat = fn_visualization(df_feb, ['상품군'], 'rank') # 상품군 매출액 순위
    dfr_prod = fn_visualization(df_feb, ['상품명','보험회사'], 'rank') # 보험상품 매출액 순위

    # 상품군별 상위 TOP5 보험상품
    dfr_cat_prod = fn_visualization(df_feb, ['상품명','보험회사','상품군'], 'rank')
    ...
    dfr_fa_prod = fn_visualization(df_feb, ['상품명','보험회사','담당자코드','담당자'], 'rank')
