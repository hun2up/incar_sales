...
if authentication_status:
    ...
    # 상품군별 매출액
    df_product = fn_category(df_may, '상품군')
    # 소속부문별 매출액
    df_channel = fn_category(df_may, '소속')
    # 보험종목별(손생) 매출액 데이터에 합계 데이터 삽입: ['보험종목','영수/환급일','매출액']
    df_insu = fn_insurance(df_may, df_insu)

    # ----------------------------------------------------  랭킹  -----------------------------------------------------------
    # 매출액 순위 (소속부문별)
    df_rank_chn = df_may.groupby(['소속'])['영수/환급보험료'].sum().reset_index(name='매출액').sort_values(by='매출액', ascending=False)
    df_rank_chn['매출액'] = df_rank_chn['매출액'].map('{:,.0f}'.format)
    
    # 매출액 상위 TOP5 (FA)
    df_rank_fa = df_may.groupby(['파트너','담당자코드','담당자'])['영수/환급보험료'].sum().reset_index(name='매출액').sort_values(by='매출액', ascending=False)
    df_rank_fa['매출액'] = df_rank_fa['매출액'].map('{:,.0f}'.format)

    df_rank_product_fa = df_may.groupby(['파트너','담당자코드','담당자','보험회사','상품명'])['영수/환급보험료'].sum().reset_index(name='매출액').sort_values(by='매출액', ascending=False)
    df_rank_product_fa['매출액'] = df_rank_product_fa['매출액'].map('{:,.0f}'.format)

    # 매출액 상위 TOP5 (보험회사)
    df_rank_company = df_may.groupby(['보험회사'])['영수/환급보험료'].sum().reset_index(name='매출액').sort_values(by='매출액', ascending=False)
    df_rank_company['매출액'] = df_rank_company['매출액'].map('{:,.0f}'.format)
    
    # 매출액 상위 TOP5 (상품군)
    df_rank_product_group = df_may.groupby(['상품군'])['영수/환급보험료'].sum().reset_index(name='매출액').sort_values(by='매출액', ascending=False)
    df_rank_product_group['매출액'] = df_rank_product_group['매출액'].map('{:,.0f}'.format)

    # 매출액 상위 TOP5 (보험상품)
    df_rank_product = df_may.groupby(['상품군','보험회사','상품명'])['영수/환급보험료'].sum().reset_index(name='매출액').sort_values(by='매출액', ascending=False)
