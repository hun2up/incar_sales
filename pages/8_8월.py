...
df_aug = fn_call(month)

# -----------------------------------------------------  사이드바  ---------------------------------------------------------
# 사이드바 헤더
st.sidebar.header("원하는 옵션을 선택하세요")
# 사이드바 제작
insurance = fn_sidebar(df_aug,'보험종목') # 월도 선택 사이드바
company = fn_sidebar(df_aug,'보험회사') # 보험사 선택 사이드바
theme = fn_sidebar(df_aug,'상품군') # 입사연차 선택 사이드바
channel = fn_sidebar(df_aug,'소속') # 소속부문 선택 사이드바
# 데이터와 사이드바 연결
df_aug = df_aug.query(
...
if authentication_status:
    ...
    # -------------------------------------------  매출액 기준 기본 전처리  ----------------------------------------------------
    # 보험종목별 매출액
    df_insu = fn_category(df_aug, '보험종목')
    # 보험회사별 매출액
    df_company = fn_category(df_aug, '보험회사')
    # 상품군별 매출액
    df_product = fn_category(df_aug, '상품군')
    # 소속부문별 매출액
    df_channel = fn_category(df_aug, '소속')
    # 보험종목별(손생) 매출액 데이터에 합계 데이터 삽입: ['보험종목','영수/환급일','매출액']
    df_insu = fn_insurance(df_aug, df_insu)

    # ----------------------------------------------------  랭킹  -----------------------------------------------------------
    # 매출액 순위 (소속부문별)
    df_rank_chn = df_aug.groupby(['소속'])['영수/환급보험료'].sum().reset_index(name='매출액').sort_values(by='매출액', ascending=False)
    df_rank_chn['매출액'] = df_rank_chn['매출액'].map('{:,.0f}'.format)
    
    # 매출액 상위 TOP5 (FA)
    df_rank_fa = df_aug.groupby(['파트너','담당자코드','담당자'])['영수/환급보험료'].sum().reset_index(name='매출액').sort_values(by='매출액', ascending=False)
    df_rank_fa['매출액'] = df_rank_fa['매출액'].map('{:,.0f}'.format)

    df_rank_product_fa = df_aug.groupby(['파트너','담당자코드','담당자','보험회사','상품명'])['영수/환급보험료'].sum().reset_index(name='매출액').sort_values(by='매출액', ascending=False)
    df_rank_product_fa['매출액'] = df_rank_product_fa['매출액'].map('{:,.0f}'.format)

    # 매출액 상위 TOP5 (보험회사)
    df_rank_company = df_aug.groupby(['보험회사'])['영수/환급보험료'].sum().reset_index(name='매출액').sort_values(by='매출액', ascending=False)
    df_rank_company['매출액'] = df_rank_company['매출액'].map('{:,.0f}'.format)
    
    # 매출액 상위 TOP5 (상품군)
    df_rank_product_group = df_aug.groupby(['상품군'])['영수/환급보험료'].sum().reset_index(name='매출액').sort_values(by='매출액', ascending=False)
    df_rank_product_group['매출액'] = df_rank_product_group['매출액'].map('{:,.0f}'.format)

    # 매출액 상위 TOP5 (보험상품)
    df_rank_product = df_aug.groupby(['상품군','보험회사','상품명'])['영수/환급보험료'].sum().reset_index(name='매출액').sort_values(by='매출액', ascending=False)
